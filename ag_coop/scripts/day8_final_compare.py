#!/usr/bin/env python3
"""
Day8 Step 5 (Final): Greedy vs Coverage 对比实验

使用双轨 SNR 指标：
- all: 包含 carrier（legacy）
- nc (non-carrier): 排除 carrier（主指标）

验收标准：
1. greedy 的 outage_nc_percent > 0（至少 5%）
2. coverage 的 outage_nc_percent 显著低于 greedy（相对降幅 ≥30%）
3. tasks_completed 不塌陷（下降 <20%）
"""
...
def main():
    print("=" * 70)
    print("Day8 Step 5 (Final): Greedy vs Coverage 对比实验")
    print("=" * 70)
    print()
    ...
    # 调整通信参数，使用严格的 SNR 阈值
    # 根据 DEVLOG 建议，使用 -12 dB 作为主阈值
    base_config['comm']['enabled'] = True
    base_config['comm']['snr_threshold_db'] = -12.0  # 严格阈值
    base_config['comm']['tx_power_db'] = -6.0  # 恢复默认发射功率
    ...
    print(f"发射功率: {base_config['comm']['tx_power_db']} dB")
    print()
    ...
    for method in methods:
        ...
        for seed in seeds:
            ...
            out_dir = Path('outputs') / f"day8_final_{method}_seed{seed}"
    ...
    greedy_outage_nc_avg = sum(r['outage_percent_nc'] for r in greedy_results) / n
    ...
    coverage_outage_nc_avg = sum(r['outage_percent_nc'] for r in coverage_results) / n
    ...
    # 打印汇总表
    print(f"{'Method':<12} {'Tasks':>8} {'Outage_NC%':>12} {'MissRate%':>10} {'Motion':>10}")
    print("-" * 70)
    print(f"{'greedy':<12} {greedy_tasks_avg:>8.1f} {greedy_outage_nc_avg:>12.1f} "
          f"{greedy_miss_rate_avg:>10.1f} {greedy_motion_avg:>10.3f}")
    print(f"{'coverage':<12} {coverage_tasks_avg:>8.1f} {coverage_outage_nc_avg:>12.1f} "
          f"{coverage_miss_rate_avg:>10.1f} {coverage_motion_avg:>10.3f}")
    print()

    # 计算改善
    outage_abs_reduction = greedy_outage_nc_avg - coverage_outage_nc_avg
    if greedy_outage_nc_avg > 0:
        outage_rel_reduction = (outage_abs_reduction / greedy_outage_nc_avg * 100)
    else:
    ...
    print(f"  绝对降幅: {outage_abs_reduction:+.2f}% (greedy {greedy_outage_nc_avg:.1f}% → coverage {coverage_outage_nc_avg:.1f}%)")
    ...
    # 标准 0: Greedy 产生 outage_nc
    greedy_has_outage = greedy_outage_nc_avg >= 5.0
    print(f"0. Greedy 产生 outage_nc (≥5%):")
    print(f"   - Greedy outage_nc: {greedy_outage_nc_avg:.1f}%")
    print(f"   结果: {'✅ PASS' if greedy_has_outage else '❌ FAIL'}")
    print()

    # 标准 1: Outage 显著降低
    outage_pass = (outage_rel_reduction >= 30) or (outage_abs_reduction >= 5)
    print(f"1. Outage_NC 显著降低:")
    ...
    # 总体结果
    overall_pass = greedy_has_outage and outage_pass and tasks_pass
    print("=" * 70)
    if overall_pass:
        print("✅✅✅ 验收通过！Coverage 方法显著改善 Outage_NC，且 Tasks 未塌陷 ✅✅✅")
    else:
        print("❌ 验收未通过")
        print()
        print("分析：")
        if not greedy_has_outage:
            print("  - Greedy 的 outage_nc < 5%，阈值可能仍然太宽松")
            print("  - 建议：使用更严格的阈值（例如 -7 dB）")
        if not outage_pass:
            print("  - Coverage 没有显著降低 outage_nc")
            print("  - 可能原因：relay 策略不够有效，或参数需要调整")
        if not tasks_pass:
            print("  - Tasks 塌陷超过 20%")
            print("  - 可能原因：relay UGV 过度追求通信，忽略了任务")
    print("=" * 70)
    print()
    ...
    report = {
        ...
        'acceptance': {
            'greedy_has_outage': greedy_has_outage,
